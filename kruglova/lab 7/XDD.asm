...
DATASG  SEGMENT  PARA 'Data'
	KEEP_CS DW 0 ;
   	...
   	MES2 DB 'From string to register and for visual from register to string: $'
	STRING DB 35 DUP('0')
DATASG	ENDS
...
START:
	...
	JGE POSITIVE_NUMBER         ; Если ax >= 0, то число положительное
	MOV STRING[di], '-'         ; Если ax < 0, то число отрицательное
	INC di
	NOT ax
...
END_CONVERT:
	MOV dx, offset STRING       ; Записываем в dx сдвиг до строки
	ret
...
START_B2D:
	...
	MOV SIGN_NUMBER, 1          ; Если равен, то число отрицательное
...
CHECK_NEGATIVE:
	...
	NOT ax;                     ; Если число отрицательное, записываем в доп. коде
	INC ax;                     ; Инвертируем и добавляем единицу
